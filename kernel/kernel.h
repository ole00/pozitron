...
#define __KERNEL_H_
	...
	*/
	...
	struct MixerWaveChannelStruct
	{
		...
		uint16_t step;				//8:8 fixed point
		uint16_t positionFrac;		//8bit fractional part 
		uint16_t position;			//sample pointer	
	};
	...
	{
		uint8_t  volume;				//(0-255)
		uint8_t  params;				//bit0=7/15 bits lfsr, b1:6=divider (samples between shifts)
		uint16_t  barrel;				//16bit LFSR barrel shifter
		uint8_t  divider;				//divider accumulator
		uint16_t  reserved;
	};
	...
	struct MixerChannelStruct
	{
		uint8_t   volume;				//(0-255)
		uint16_t  step;					//8:8 fixed point
		unsigned const char structAlignment[4];	//dont access!
	};
	...
	extern uint16_t mix_buf[];
	extern volatile uint16_t *mix_pos;
	...
	extern void ProcessMusic();
	extern void MixTracks(uint16_t stage);
