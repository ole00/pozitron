...
//Draws a map of tile at the specified position
	...
	void DrawMap(unsigned char x,unsigned char y,const VRAM_PTR_TYPE *map) {
		...
		for(u8 dy = 0; dy < mapHeight; dy++) {
			for(u8 dx = 0; dx < mapWidth; dx++) {			
				SetTile(x + dx, y + dy, pgm_read_byte(&(map[posY + dx])));					
			}
			posY += mapWidth;
		}
	...
	void DrawMap(unsigned char x,unsigned char y,const VRAM_PTR_TYPE *map){
		uint16_t posY = 2; 
		uint16_t mapWidth = pgm_read_word(&(map[0]));
		uint16_t mapHeight = pgm_read_word(&(map[1]));

		for(u8 dy = 0; dy < mapHeight; dy++){
			for(u8 dx = 0; dx < mapWidth; dx++){			
				SetTile(x + dx, y + dy, pgm_read_word(&(map[posY + dx])));	
			}
			posY += mapWidth;
		}
	}
