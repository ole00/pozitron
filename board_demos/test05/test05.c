...
uint16_t dma_buffer[1024] __attribute__((aligned (32)));
...
static void enableTimerInterruptHandler()
{
...
static void enableDmaInterruptHandler(void) {
...
static void setupPixelDma()
{
    ...
    DMA1_Channel5->CMAR = (uint32_t)dma_buffer; // use this buffer source buffer
...
static void setupDmaBuffer(void) {
    ...
    // fill the DMA buffer with some values
    for (i = 0; i < VGA_MODE; i++) {
        dma_buffer[i] = i;
    }

    dma_buffer[VGA_MODE] = 0;
}
...
int main(void)
{
	...
	// Keep running, the PWM timer and DMA will drive the GPIO pins.
	while (1) {
		NOP;
	}
}
